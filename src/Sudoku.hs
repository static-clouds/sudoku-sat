...
toMoves s = [(C { row = row, col = col, val = fromJust val}) | (row, col) <- allPositions gridSize
...
cellRule values cell@(C { val = val }) = makeRule (matchesVal val) $ cellValues values cell
...
rowRule values cell@(C { col = col }) = makeRule (matchesCol col) $ rowValues values cell
...
colRule values cell@(C { row = row }) = makeRule (matchesRow row) $ colValues values cell
...
boxRule gridSize cell@(C { row = row, col = col }) = makeRule (matchesRowCol row col) $ boxValues (box gridSize cell) cell
